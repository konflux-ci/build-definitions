...
build_push_task() {
    ...
    # while read -r extra_annotation; do
    #     if [ -n "$extra_annotation" ]; then
    #         ANNOTATIONS+=("$extra_annotation")
    #     fi
    ...
    local -r digest="${output##*$'\n'}"

    cache_set "${task_bundle}-${task_file_sha}" "${digest}"
    echo "$digest"
}
...
attach_migration_file() {
    ...
    (
        ...
        retry oras attach \
            ...
            --annotation "$ANNOTATION_TASK_MIGRATION" \
