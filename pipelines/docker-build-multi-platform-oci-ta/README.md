...
|buildah-format| The format for the resulting image's mediaType. Valid values are oci or docker.| docker| build-images:BUILDAH_FORMAT ; build-image-index:BUILDAH_FORMAT|
|dockerfile| Path to the Dockerfile inside the context specified by parameter path-context| Dockerfile| build-images:DOCKERFILE ; push-dockerfile:DOCKERFILE|
...
|hermetic| Execute the build with network isolation| false| build-images:HERMETIC|
|image-expires-after| Image tag expiration time, time values could be something like 1h, 2d, 3w for hours, days, and weeks, respectively.| | clone-repository:ociArtifactExpiresAfter ; prefetch-dependencies:ociArtifactExpiresAfter ; build-images:IMAGE_EXPIRES_AFTER|
...
|output-image| Fully Qualified Output Image| None| clone-repository:ociStorage ; prefetch-dependencies:ociStorage ; build-images:IMAGE ; build-image-index:IMAGE|
|path-context| Path to the source code of an application's component from where to build image.| .| build-images:CONTEXT ; push-dockerfile:CONTEXT|
|prefetch-input| Build dependencies to be prefetched| | prefetch-dependencies:input ; build-images:PREFETCH_INPUT|
|privileged-nested| Whether to enable privileged mode, should be used only with remote VMs| false| build-images:PRIVILEGED_NESTED|
...
### push-dockerfile-oci-ta task parameters
|name|description|default value|already set by|
|---|---|---|---|
...
### source-build-oci-ta task parameters
|name|description|default value|already set by|
|---|---|---|---|
...
|name|description|used in params (taskname:taskparam)
|---|---|---|
|CACHI2_ARTIFACT| The Trusted Artifact URI pointing to the artifact with the prefetched dependencies.| build-images:CACHI2_ARTIFACT ; build-source-image:CACHI2_ARTIFACT ; sast-snyk-check:CACHI2_ARTIFACT ; sast-shell-check:CACHI2_ARTIFACT ; sast-unicode-check:CACHI2_ARTIFACT|
|SOURCE_ARTIFACT| The Trusted Artifact URI pointing to the artifact with the application source code.| build-images:SOURCE_ARTIFACT ; build-source-image:SOURCE_ARTIFACT ; sast-snyk-check:SOURCE_ARTIFACT ; sast-shell-check:SOURCE_ARTIFACT ; sast-unicode-check:SOURCE_ARTIFACT ; push-dockerfile:SOURCE_ARTIFACT|
### push-dockerfile-oci-ta task results
|name|description|used in params (taskname:taskparam)
|---|---|---|
...
|---|---|---|
|TEST_OUTPUT| Tekton task test output.| |
### source-build-oci-ta task results
|name|description|used in params (taskname:taskparam)
|---|---|---|
