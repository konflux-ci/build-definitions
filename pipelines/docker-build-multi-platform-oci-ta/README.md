...
|---|---|---|---|
|build-args| Array of --build-arg values ("arg=value" strings) for buildah| []| build-images:0.8:BUILD_ARGS ; sast-coverity-check:0.3:BUILD_ARGS|
|build-args-file| Path to a file with build arguments for buildah, see https://www.mankier.com/1/buildah-build#--build-arg-file| | build-images:0.8:BUILD_ARGS_FILE ; sast-coverity-check:0.3:BUILD_ARGS_FILE|
...
|buildah-format| The format for the resulting image's mediaType. Valid values are oci or docker.| docker| build-images:0.8:BUILDAH_FORMAT ; build-image-index:0.2:BUILDAH_FORMAT|
|dockerfile| Path to the Dockerfile inside the context specified by parameter path-context| Dockerfile| build-images:0.8:DOCKERFILE ; sast-coverity-check:0.3:DOCKERFILE ; push-dockerfile:0.1:DOCKERFILE|
|enable-cache-proxy| Enable cache proxy configuration| false| init:0.3:enable-cache-proxy|
|git-url| Source Repository URL| None| clone-repository:0.1:url|
|hermetic| Execute the build with network isolation| false| build-images:0.8:HERMETIC ; sast-coverity-check:0.3:HERMETIC|
|image-expires-after| Image tag expiration time, time values could be something like 1h, 2d, 3w for hours, days, and weeks, respectively.| | clone-repository:0.1:ociArtifactExpiresAfter ; prefetch-dependencies:0.2:ociArtifactExpiresAfter ; build-images:0.8:IMAGE_EXPIRES_AFTER ; build-image-index:0.2:IMAGE_EXPIRES_AFTER ; sast-coverity-check:0.3:IMAGE_EXPIRES_AFTER|
|output-image| Fully Qualified Output Image| None| clone-repository:0.1:ociStorage ; prefetch-dependencies:0.2:ociStorage ; build-images:0.8:IMAGE ; build-image-index:0.2:IMAGE ; sast-coverity-check:0.3:IMAGE|
|path-context| Path to the source code of an application's component from where to build image.| .| build-images:0.8:CONTEXT ; sast-coverity-check:0.3:CONTEXT ; push-dockerfile:0.1:CONTEXT|
|prefetch-input| Build dependencies to be prefetched| | prefetch-dependencies:0.2:input ; build-images:0.8:PREFETCH_INPUT ; sast-coverity-check:0.3:PREFETCH_INPUT|
|privileged-nested| Whether to enable privileged mode, should be used only with remote VMs| false| build-images:0.8:PRIVILEGED_NESTED|
...
|commit| The precise commit SHA that was fetched by this Task.| build-images:0.8:COMMIT_SHA ; build-images:0.8:SOURCE_DATE_EPOCH ; build-image-index:0.2:COMMIT_SHA ; sast-coverity-check:0.3:COMMIT_SHA|
|commit-timestamp| The commit timestamp of the checkout| build-images:0.8:SOURCE_DATE_EPOCH|
...
|name|description|used in params (taskname:taskrefversion:taskparam)
|---|---|---|
|CACHI2_ARTIFACT| The Trusted Artifact URI pointing to the artifact with the prefetched dependencies.| build-images:0.8:CACHI2_ARTIFACT ; build-source-image:0.3:CACHI2_ARTIFACT ; sast-snyk-check:0.4:CACHI2_ARTIFACT ; sast-coverity-check:0.3:CACHI2_ARTIFACT ; sast-shell-check:0.1:CACHI2_ARTIFACT ; sast-unicode-check:0.4:CACHI2_ARTIFACT|
|SOURCE_ARTIFACT| The Trusted Artifact URI pointing to the artifact with the application source code.| build-images:0.8:SOURCE_ARTIFACT ; build-source-image:0.3:SOURCE_ARTIFACT ; sast-snyk-check:0.4:SOURCE_ARTIFACT ; sast-coverity-check:0.3:SOURCE_ARTIFACT ; sast-shell-check:0.1:SOURCE_ARTIFACT ; sast-unicode-check:0.4:SOURCE_ARTIFACT ; push-dockerfile:0.1:SOURCE_ARTIFACT|
