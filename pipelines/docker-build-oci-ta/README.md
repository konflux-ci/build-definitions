...
|dockerfile| Path to the Dockerfile inside the context specified by parameter path-context| Dockerfile| build-container:DOCKERFILE ; push-dockerfile:DOCKERFILE|
...
|path-context| Path to the source code of an application's component from where to build image.| .| build-container:CONTEXT ; push-dockerfile:CONTEXT|
...
### buildah-oci-ta task parameters
|name|description|default value|already set by|
|---|---|---|---|
...
|name|description|used in params (taskname:taskparam)
|---|---|---|
|CACHI2_ARTIFACT| The Trusted Artifact URI pointing to the artifact with the prefetched dependencies.| build-container:CACHI2_ARTIFACT ; build-source-image:CACHI2_ARTIFACT ; sast-snyk-check:CACHI2_ARTIFACT ; sast-shell-check:CACHI2_ARTIFACT ; sast-unicode-check:CACHI2_ARTIFACT|
|SOURCE_ARTIFACT| The Trusted Artifact URI pointing to the artifact with the application source code.| build-container:SOURCE_ARTIFACT ; build-source-image:SOURCE_ARTIFACT ; sast-snyk-check:SOURCE_ARTIFACT ; sast-shell-check:SOURCE_ARTIFACT ; sast-unicode-check:SOURCE_ARTIFACT ; push-dockerfile:SOURCE_ARTIFACT|
