...
|---|---|---|---|
|build-args| Build args for ko| | build-container:0.1:BUILD_ARGS|
...
|hermetic| Execute the build with network isolation| false| build-container:0.1:HERMETIC ; generate-code:0.1:HERMETIC|
|image-expires-after| Image tag expiration time, time values could be something like 1h, 2d, 3w for hours, days, and weeks, respectively.| | clone-repository:0.1:ociArtifactExpiresAfter ; prefetch-dependencies:0.2:ociArtifactExpiresAfter ; build-container:0.1:IMAGE_EXPIRES_AFTER ; build-image-index:0.1:IMAGE_EXPIRES_AFTER ; generate-code:0.1:ociArtifactExpiresAfter|
|import-path| Import path to build| | build-container:0.1:IMPORT_PATH|
|output-image| Fully Qualified Output Image| None| init:0.2:image-url ; clone-repository:0.1:ociStorage ; prefetch-dependencies:0.2:ociStorage ; build-container:0.1:IMAGE ; build-image-index:0.1:IMAGE ; generate-code:0.1:ociStorage|
|path-context| Path to the source code of an application's component from where to build image.| .| |
|prefetch-input| Build dependencies to be prefetched| | prefetch-dependencies:0.2:input|
|preprocessing-script| Preprocessing script for source code| echo 'No preprocessing script, skipping.'| generate-code:0.1:SCRIPT|
...
|name|description|default value|already set by|
|---|---|---|---|
|BUILD_ARGS| additional arguments to ko build| | '$(params.build-args)'|
...
|IMAGE| Reference of the image ko will produce.| None| '$(params.output-image)'|
...
|KO_IMAGE| ko image to use for builds| quay.io/redhat-user-workloads/konflux-build-pipeli-tenant/ko-builder:127e70d57df2a6b75bf3a2bb233a6223fb6a8cab| 'quay.io/gallen/ko-builder:latest'|
|SOURCE_ARTIFACT| The Trusted Artifact URI pointing to the artifact with the application source code.| None| '$(tasks.generate-code.results.SCRIPT_ARTIFACT)'|
|caTrustConfigMapKey| The name of the key in the ConfigMap that contains the CA bundle data.| ca-bundle.crt| |
|caTrustConfigMapName| The name of the ConfigMap to read CA bundle data from.| trusted-ca| |
...
### run-script-oci-ta:0.1 task parameters
|name|description|default value|already set by|
|---|---|---|---|
|HERMETIC| Determines if build will be executed without network access.| false| '$(params.hermetic)'|
|PREFETCH_ARTIFACT| The Trusted Artifact URI pointing to the artifact with the prefetched dependencies.| | |
|SCRIPT| The script to launch| None| '$(params.preprocessing-script)'|
|SCRIPT_ARTIFACT_RELATIVE_PATH| The relative path relative to the source directory to store on the SCRIPT_ARTIFACT| .| |
|SCRIPT_RUNNER_IMAGE| The image to run the script in| None| 'quay.io/konflux-ci/operator-sdk-builder:latest'|
|SCRIPT_WORKDIR| The directory to launch the script from| /var/workdir/source| |
...
|enableSymlinkCheck| Check symlinks in the repo. If they're pointing outside of the repo, the build will fail. | true| |
|ociArtifactExpiresAfter| Expiration date for the trusted artifacts created in the OCI repository. An empty string means the artifacts do not expire.| | '$(params.image-expires-after)'|
|ociStorage| The OCI repository where the Trusted Artifacts are stored.| None| '$(params.output-image).script'|
...
|name|description|used in params (taskname:taskrefversion:taskparam)
|---|---|---|
|CACHI2_ARTIFACT| The Trusted Artifact URI pointing to the artifact with the prefetched dependencies.| build-container:0.1:CACHI2_ARTIFACT ; build-source-image:0.3:CACHI2_ARTIFACT ; sast-snyk-check:0.4:CACHI2_ARTIFACT ; sast-shell-check:0.1:CACHI2_ARTIFACT ; sast-unicode-check:0.3:CACHI2_ARTIFACT|
|SOURCE_ARTIFACT| The Trusted Artifact URI pointing to the artifact with the application source code.| generate-code:0.1:SOURCE_ARTIFACT|
...
|TEST_OUTPUT| Tekton task test output.| |
### run-script-oci-ta:0.1 task results
|name|description|used in params (taskname:taskrefversion:taskparam)
|---|---|---|
|SCRIPT_ARTIFACT| The Trusted Artifact URI pointing to the artifact with the application source code.| build-container:0.1:SOURCE_ARTIFACT ; build-source-image:0.3:SOURCE_ARTIFACT ; sast-snyk-check:0.4:SOURCE_ARTIFACT ; sast-shell-check:0.1:SOURCE_ARTIFACT ; sast-unicode-check:0.3:SOURCE_ARTIFACT|
