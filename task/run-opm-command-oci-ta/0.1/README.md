...
3.  **Applies ImageDigestMirrorSet (Optional)**: If an `ImageDigestMirrorSet` (IMDS) file is provided, this step replaces image pull specifications in the generated OPM output (e.g., in a catalog) based on the mirror definitions in the IMDS file. This is crucial for environments where images need to be pulled from internal registries.
...
| `IMDS_PATH`         | Path to an `ImageDigestMirrorSet` file (e.g., `.tekton/images-mirror-set.yaml`). Used to replace related image pullspecs in the catalog.                          | `string`| No       | `""` (empty string) |
