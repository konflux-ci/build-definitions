...
if [ -n "$REUSED_IMAGE_REF" ]; then
  echo "Artifact is being reused, reusing existing SBOM"
  ...
  # Download the existing SBOM from the reused image
  if cosign download sbom "$REUSED_IMAGE_REF" > existing-sbom.json 2>/dev/null; then
