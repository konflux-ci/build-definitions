...
' | kubectl apply -f -
  ...
  It 'fails when STEPS_IMAGE_STEP_NAMES mixes include and exclude entries'
    build_mixed_include_exclude() {
      restore_source_files
      local tkn_out
      # tkn task start exits 0 even when the task step fails, so capture output
      # and verify the TaskRun status directly
      tkn_out="$(tkn task start tkn-bundle -p IMAGE=registry:5000/bundle:mixed-fail -p STEPS_IMAGE=registry.io/repository/replaced:latest -p STEPS_IMAGE_STEP_NAMES=build,!test -p URL=https://example.com -p REVISION=main --use-param-defaults --timeout 15m --showlog -w name=source,claimName=source-pvc 2>&1)" || true
      printf '%s\n' "$tkn_out"
      # Extract the TaskRun name from "TaskRun started: <name>" and check its status
      local tr_name
      tr_name="$(printf '%s\n' "$tkn_out" | awk '/^TaskRun started:/{print $3; exit}')"
      if [[ -n "$tr_name" ]]; then
        printf 'TASK_FAILED=%s\n' "$(tkn tr describe "$tr_name" -o jsonpath='{.status.conditions[?(@.type=="Succeeded")].reason}' 2>/dev/null)"
      else
        echo "DEBUG: could not extract TaskRun name from output" >&2
        echo "DEBUG: first line of output: $(printf '%s\n' "$tkn_out" | head -1)" >&2
      fi
    }

    When call build_mixed_include_exclude
    The output should include 'STEPS_IMAGE_STEP_NAMES cannot mix include and exclude (!) entries'
  ...
  It 'replaces named step images with space-separated STEPS_IMAGE_STEP_NAMES'
    build_and_inspect_space_sep() {
      restore_source_files
      tkn task start tkn-bundle -p IMAGE=registry:5000/bundle:space-sep -p STEPS_IMAGE=registry.io/repository/replaced:latest -p "STEPS_IMAGE_STEP_NAMES=build test" -p URL=https://example.com -p REVISION=main --use-param-defaults --timeout 15m --showlog -w name=source,claimName=source-pvc
      tkn bundle list -o=go-template --template '{{range .spec.steps}}{{printf "%s=%s\n" .name .image}}{{end}}' localhost:5000/bundle:space-sep 2>/dev/null
    }

    When call build_and_inspect_space_sep
    The output should include 'Added Task: test1 to image'
    The output should include 'Pushed Tekton Bundle to registry:5000/bundle'
    # In test1.yaml: both build and test steps should be replaced
    The output should include 'build=registry.io/repository/replaced:latest'
    The output should include 'test=registry.io/repository/replaced:latest'
    # In test2.yml: test2-step should NOT be replaced
    The output should include 'test2-step=ubuntu'
    # In test3.yaml: test3-step should NOT be replaced
    The output should include 'test3-step=ubuntu'
  End
End
 ...
 # Manual spot-checking: after a test run, the pushed bundles remain in the
  # local registry. Inspect any bundle with:
  #   tkn bundle list localhost:5000/bundle:<tag>
  # For example:
  #   tkn bundle list localhost:5000/bundle:all-replaced
  #   tkn bundle list localhost:5000/bundle:selective-one
  #   tkn bundle list localhost:5000/bundle:selective-multi
