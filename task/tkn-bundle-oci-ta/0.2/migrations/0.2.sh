...
if yq -e '.spec.tasks[] | select(.name == "tkn-bundle")' "$pipeline_file" >/dev/null && ! yq -e '.spec.tasks[] | select(.name == "clone-repository").params[] | select(.name == "depth")' "$pipeline_file" >/dev/null; then
...
elif yq -e '.spec.tasks[] | select(.name == "tkn-bundle-oci-ta")' "$pipeline_file" >/dev/null && ! yq -e '.spec.tasks[] | select(.name == "clone-repository").params[] | select(.name == "depth")' "$pipeline_file" >/dev/null; then
